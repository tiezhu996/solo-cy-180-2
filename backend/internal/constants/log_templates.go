...
const (
	...
	LogRoleDenied       = "role denied user=%s role=%s entity=%s action=%s"
	LogOwnerDenied      = "ownership denied user=%s project=%d owner=%d action=%s"
	LogArchivedWrite    = "archived write rejected user=%s project=%d entity=%s action=%s"
	LogCrossProject     = "cross project write rejected user=%s project=%d child_project=%d entity=%s"
	LogRoleEscalation   = "role escalation rejected via public register username=%s role=%s"
)
